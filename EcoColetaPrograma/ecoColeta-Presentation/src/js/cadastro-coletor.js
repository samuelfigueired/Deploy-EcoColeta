...
document.addEventListener("DOMContentLoaded", function() {
    ...
    async function buscarCoordenadas(cep, endereco, cidade, estado) {
        try {
            ...
            if (data && data.length > 0) {
                ...
                console.warn("Não foi possível encontrar coordenadas exatas, usando coordenadas padrão de Betim");
                return {
                    lat: -19.9677,
                    lng: -44.1986
                };
    ...
    form.addEventListener("submit", async function(event) {
        ...
        try {
            ...
            const emailCheckResponse = await fetch(`http://localhost:3000/usuarios?email=${encodeURIComponent(email)}`);
            const usuariosExistentes = await emailCheckResponse.json();
            
            if (usuariosExistentes.length > 0) {
                alert("Este email já está cadastrado no sistema. Tente fazer login ou use outro email.");
                return;
            }

            // Busca coordenadas para o ecoponto
            const coordenadas = await buscarCoordenadas(cep, endereco, cidade, estado);
            ...
            // Cadastra o ecoponto
            const ecopontoResponse = await fetch("http://localhost:3000/pontosDeColeta", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
            ...
            await fetch(`http://localhost:3000/usuarios/${novoUsuario.id}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json"
                },
